...
////////////////////////////////////////////////////////////////////////////////
    ...
    filter[0][0][0] = 1;
    gaussian(filter[1],1,filtRows[1],filtCols[1]);
    gabor(filter[2],2,0,filtRows[2],filtCols[2]);
    gabor(filter[3],2,90,filtRows[3],filtCols[3]);
    gabor(filter[4],3,0,filtRows[5],filtCols[4]);
    gabor(filter[5],3,90,filtRows[5],filtCols[5]);
    gabor(filter[6],1,0,filtRows[6],filtCols[6]);
    gabor(filter[7],1,90,filtRows[7],filtCols[7]);
    ...
    for(i=0;i<noFilts;i++)
        lambdas[i] = malloc(128*sizeof(*lambdas[i]));
    ...
    } while ((diff > min || loopNo<=6500) && loopNo<500);
...
void localHistogramIn(int* histogram, int** img, int imgRows, int imgCols, int a, int b){
    ...
    for (x=0;x<128;x++)
    {
        if (histogram[x] < 0 || histogram[x] > ((imgRows-4)*(imgCols-4)))
        {
            printf("BROKEN HISTOGRAM: histogram[%d] = %d ",x,histogram[x]);
        }
    }
}
