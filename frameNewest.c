...
////////////////////////////////////////////////////////////////////////////////
    ...
    do{
        ...
        for (m=0;m<noFilts;m++){
            for (i=0;i<8;i++){
                //lambdas[m][i] = lambdas[m][i] + temp * (synthHistogram[m][i] - obsHistogram[m][i]);
            ...
            for (i=2;i<rows-2;i++){
                for (j=2;j<cols-2;j++){
                    ...
                    if (current < curEnergy){
                        ...
                        prob = exp(-(loopNo*10)*(current - curEnergy));
                        //prob =0;
    ...
    } while ((diff > min || loopNo<=450) && loopNo<500);
