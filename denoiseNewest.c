...
////////////////////////////////////////////////////////////////////////////////
    ...
    // lambda stuff
    //float tempLambdas[noFilts][8] = {{-179.704666,166.140167,86.277519,47.956821,31.464453,37.660759,63.511303,-253.306107},
    //                                {60.171928,53.694286,47.393303,36.767731,35.618591,45.152855,16.318546,-147.018311}};

    float tempLambdas[2][8] = {{65.279266,113.055473,77.744293,67.823204,72.761826,85.636070,131.546402,-613.846191},
                                    {123.728203,172.308472,75.352287,78.892670,75.786316,157.575485,95.472664,-614.984619}};
    ...
    for (k=1;k<=200;k+=1){
        ...
        curEnergy = energyMatch(rows, cols, synthImg, img) + lambda * curEnergy;
        ...
        for (i=2;i<rows-2;i++){
            for (j=2;j<cols-2;j++){
                ...
                current = energyMatch(rows, cols, imgCopy1, img) + lambda * current;
                ...
                if (current < curEnergy){
                    ...
                    prob = exp(-beta*(current - curEnergy));
                    prob =0;
