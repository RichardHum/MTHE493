...
////////////////////////////////////////////////////////////////////////////////
    ...
    gaussian(filter[2],2,filtRows[2],filtCols[2]);
    ...
    for (i=1;i<noFilts;i++)
    {
        ...
        normalizeHistogram(rows,cols,obsHistogram[m],normObsHistogram[m]);
    }
            ...
            for (i=2;i<rows-2;i++){
                for (j=2;j<cols-2;j++){
                    ...
                    if (current < curEnergy){
                        ...
                        for(l=1;l<32;l++){
                            probs[l] = (energies[l]-minNRG);
                            probSums += probs[l];
                        }

                        probs[randInt] = probs[randInt] * 10;
                        probs[synthImg[i][j]] = probs[synthImg[i][j]] * 10;

                        //prob = exp(-1*(probs[randInt]-probs[synthImg[i][j]]));*/

                        randomNumber = rand();
                        randomNumber = randomNumber/RAND_MAX;
                        //prob = exp(-(1/beta) * (current - curEnergy)*500);
                        prob = exp(-100*(current - curEnergy));
                        if(prob>1)
                            prob = 1;
    ...
    } while ((diff > min || loopNo<=4000) && loopNo<5000);
...
void normalizeHistogram(int imgRows, int imgCols, int* histIn, float* histOut)
{
    ...
    for (i=0;i<32;i++)
    {
        histOut[i] = histIn[i] / ((float)(imgRows * imgCols)/2);
    }
}
